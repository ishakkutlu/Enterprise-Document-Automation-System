...
'Tek sendkeys F10 komutu mousemove prosedürülerini kapattýðý için, ikincisinde f10 görevi iptal ediliyor.
Application.SendKeys "{F10}", True
...
'ÖNEMLÝ: userform numlock true/false deðerini vbModeless iken alýyor; vbModal iken numlock boolean deðeri alýnamýyor.
'Bu yüzden userform vbmodeless modunda açýlýrken, userform vbModal olmadan önce NumLockAc prosedürü çaðrýlýp numlock'un boolean deðeri elde edilmiþ olunuyor.

'Numlock open olmasý için numlocku etkileyen kodlardan önce ve sonra yukarýdaki komutlar kullanýlýr.
'Numlock açýksa açýk, kapalý ise kapalý kalýyor. Ancak userform aktifken numlock açýlýr veya kapatýlýr ise açýlýp kapanma oluyor.
'Userformu sað üst köþeden küçültüp numlock açýlýr. Tekrar ayný buton ile userform geniþletilir ve sorun çözülmüþ olur.
