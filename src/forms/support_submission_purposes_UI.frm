VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} support_submission_purposes_UI 
   Caption         =   "Enterprise Document Automation System"
   ClientHeight    =   8895
   ClientLeft      =   30
   ClientTop       =   375
   ClientWidth     =   13920
   OleObjectBlob   =   "support_submission_purposes_UI.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "support_submission_purposes_UI"
...
Private Sub ComboAmac_DblClick(ByVal Cancel As MSForms.ReturnBoolean)
Me.ComboAmac.DropDown
End Sub
...
LabelEkle.ForeColor = RGB(256, 256, 256)
End Sub
...
LabelKaldir.ForeColor = RGB(256, 256, 256)
End Sub
...
End Sub
Private Sub ComboAmac_MouseMove(ByVal Button As Integer, ByVal Shift As Integer, ByVal x As Single, ByVal y As Single)
Call ColorChangerGenel
Call SetComboBoxHook(ComboAmac) 'Open scrollable with mouse
End Sub
...
If ItemName <> "" Then
    ...
    ItemName = LCase(Replace(Replace(ItemName, "I", "i"), "I", "ı"))
    'Comboya tanımlı değer girilemez.(Rezerv tanımları için)
    a() = ComboAmac.List
    For i = LBound(a) To UBound(a)
        If a(i, 0) = ItemName Then
            MsgBox "The purpose of delivery for '" & ItemName & "' is already defined in the dropdown list. The operation was not completed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
            GoTo Son
        End If
    Next i
Else
...
SayHedef = Workbooks(FileName).Worksheets(1).Range("DM1000").End(xlUp).Row
If SayHedef < 6 Then
    SayHedef = 6
End If
If SayHedef > 54 Then
    MsgBox "The dropdown list for delivery purpose definitions is full. The delivery unit '" & ItemName & "' could not be added.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    Workbooks(FileName).Close SaveChanges:=True
    GoTo Son
End If
...
If SayHedef > 6 Then
    For j = 6 To SayHedef
        If Workbooks(FileName).Worksheets(1).Cells(j, 117).Value = "" Then
            SayHedef = j - 1
            GoTo DonguSon
        End If
    Next j
...
Workbooks(FileName).Worksheets(1).Cells(SayHedef + 1, 117).Value = ItemName
ThisWorkbook.Worksheets(2).Cells(SayHedef + 1, 117).Value = ItemName
...
If SayHedef > 6 Then
    ...
    'Sort A to Z
    Workbooks(FileName).Worksheets(1).Range("DM" & 6 & ":DM" & SayHedef).Sort key1:=Workbooks(FileName).Worksheets(1).Range("DM" & 6), order1:=xlAscending, Header:=xlNo
    ThisWorkbook.Worksheets(2).Range("DM" & 6 & ":DM" & SayHedef).Sort key1:=ThisWorkbook.Worksheets(2).Range("DM" & 6), order1:=xlAscending, Header:=xlNo
...
'ThisWorkbook.Save
MsgBox "The delivery purpose for '" & ItemName & "' was successfully added to the dropdown list.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
Dim FileName As String, ListControl As Integer, ItemBul As Range, SayHedef As Long, j As Long
...
If ListControl = 0 Then
    MsgBox "The delivery purpose for '" & ItemName & "' was not found in the dropdown list, so it could not be removed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    GoTo Son
End If
...
Set ItemBul = Workbooks(FileName).Worksheets(1).Range("DM6:DM1000").Find(What:=ItemName, SearchDirection:=xlNext, _
...
Workbooks(FileName).Worksheets(1).Cells(ItemBul.Row, 117).Value = ""
ThisWorkbook.Worksheets(2).Cells(ItemBul.Row, 117).Value = ""

''Oluşan boş satır arada ise boş satırı kaldır.
'If ItemBul.Row <> SayHedef Then
'    If SayHedef > 6 Then
'        For j = ItemBul.Row To SayHedef
'            If j < 55 Then
'                If Workbooks(FileName).Worksheets(1).Cells(j, 117).Value = "" Then
'                    Workbooks(FileName).Worksheets(1).Cells(j, 117).Value = Workbooks(FileName).Worksheets(1).Cells(j + 1, 117).Value
'                    Workbooks(FileName).Worksheets(1).Cells(j + 1, 117).Value = ""
'                    ThisWorkbook.Worksheets(2).Cells(j, 117).Value = ThisWorkbook.Worksheets(2).Cells(j + 1, 117).Value
'                    ThisWorkbook.Worksheets(2).Cells(j + 1, 117).Value = ""
'                End If
'            End If
'        Next j
'    End If
...
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The delivery purpose for '" & ItemName & "' has been successfully removed from the dropdown lists.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
For Each ClrLab In support_submission_purposes_UI.Controls
...
support_submission_purposes_UI.BackColor = RGB(230, 230, 230) 'YENİ
...
Call timeout(0.01)
    If Rep > 60 Then
        support_submission_purposes_UI.Width = Rep
        yukseklik = yukseklik - 60
        support_submission_purposes_UI.Height = yukseklik
