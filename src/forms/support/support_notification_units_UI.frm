VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} support_notification_units_UI 
   Caption         =   "Enterprise Document Automation System"
   ClientHeight    =   8895
   ClientLeft      =   30
   ClientTop       =   375
   ClientWidth     =   13920
   OleObjectBlob   =   "support_notification_units_UI.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "support_notification_units_UI"
...
Private Sub CombogecersizBirimi_DblClick(ByVal Cancel As MSForms.ReturnBoolean)
Me.CombogecersizBirimi.DropDown
End Sub
...
Private Sub CombogecersizBirimi_MouseMove(ByVal Button As Integer, ByVal Shift As Integer, ByVal x As Single, ByVal y As Single)
Call ColorChangerGenel
Call SetComboBoxHook(CombogecersizBirimi) 'Open scrollable with mouse
End Sub
...
ItemName = CombogecersizBirimi.Value
...
If ItemName <> "" Then
    ...
    a() = CombogecersizBirimi.List
    For i = LBound(a) To UBound(a)
        If a(i, 0) = ItemName Then
            MsgBox "The unit named '" & ItemName & "', subject to invalidation, has already been defined in the relevant dropdown list. The operation could not be completed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
            GoTo Son
        End If
    Next i
Else
...
SayHedef = Workbooks(FileName).Worksheets(1).Range("DP1000").End(xlUp).Row
If SayHedef < 6 Then
    SayHedef = 6
End If
If SayHedef > 304 Then
    MsgBox "The dropdown list definition area for the unit related to the notification is full. Therefore, the delivery unit information named '" & ItemName & "' could not be defined.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    Workbooks(FileName).Close SaveChanges:=True
    GoTo Son
End If
...
If SayHedef > 6 Then
    For j = 6 To SayHedef
        If Workbooks(FileName).Worksheets(1).Cells(j, 120).Value = "" Then
            SayHedef = j - 1
            GoTo DonguSon
        End If
    Next j
...
Workbooks(FileName).Worksheets(1).Cells(SayHedef + 1, 120).Value = ItemName
ThisWorkbook.Worksheets(2).Cells(SayHedef + 1, 120).Value = ItemName
...
If SayHedef > 6 Then
    ...
    'Sort A to Z
    Workbooks(FileName).Worksheets(1).Range("DP" & 6 & ":DP" & SayHedef).Sort key1:=Workbooks(FileName).Worksheets(1).Range("DP" & 6), order1:=xlAscending, Header:=xlNo
    ThisWorkbook.Worksheets(2).Range("DP" & 6 & ":DP" & SayHedef).Sort key1:=ThisWorkbook.Worksheets(2).Range("DP" & 6), order1:=xlAscending, Header:=xlNo
...
CombogecersizBirimi.Value = ""
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The unit information related to the notification named '" & ItemName & "' has been successfully added to the relevant dropdown list.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
If ListControl = 0 Then
    MsgBox "The unit information related to the notification named '" & ItemName & "' could not be removed because it was not previously assigned to the relevant dropdown list.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    GoTo Son
End If
...
Set ItemBul = Workbooks(FileName).Worksheets(1).Range("DP6:DP1000").Find(What:=ItemName, SearchDirection:=xlNext, _
...
Workbooks(FileName).Worksheets(1).Cells(ItemBul.Row, 120).Value = ""
ThisWorkbook.Worksheets(2).Cells(ItemBul.Row, 120).Value = ""
...
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The unit information related to the notification named '" & ItemName & "' has been successfully removed from the relevant dropdown lists.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
For Each ClrLab In support_notification_units_UI.Controls
...
support_notification_units_UI.BackColor = RGB(230, 230, 230) 'YENİ
...
Call timeout(0.01)
    If Rep > 60 Then
        support_notification_units_UI.Width = Rep
        yukseklik = yukseklik - 60
        support_notification_units_UI.Height = yukseklik
