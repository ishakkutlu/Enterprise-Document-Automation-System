VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} support_delivery_units_UI 
   Caption         =   "Enterprise Document Automation System"
   ClientHeight    =   8895
   ClientLeft      =   30
   ClientTop       =   375
   ClientWidth     =   13920
   OleObjectBlob   =   "support_delivery_units_UI.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "support_delivery_units_UI"
...
Private Sub ComboTeslimatBirimi_DblClick(ByVal Cancel As MSForms.ReturnBoolean)
Me.ComboTeslimatBirimi.DropDown
End Sub
...
Private Sub ComboTeslimatBirimi_MouseMove(ByVal Button As Integer, ByVal Shift As Integer, ByVal x As Single, ByVal y As Single)
Call ColorChangerGenel
Call SetComboBoxHook(ComboTeslimatBirimi) 'Open scrollable with mouse
End Sub
...
ItemName = ComboTeslimatBirimi.Value
...
If ItemName <> "" Then
    ...
    a() = ComboTeslimatBirimi.List
    For i = LBound(a) To UBound(a)
        If a(i, 0) = ItemName Then
            MsgBox "The delivery/financial unit information named " & ItemName & " has already been defined for the related dropdown list, and therefore the operation could not be completed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
            GoTo Son
        End If
    Next i
Else
...
SayHedef = Workbooks(FileName).Worksheets(1).Range("DO1000").End(xlUp).Row
If SayHedef < 7 Then
    SayHedef = 7
End If
If SayHedef > 304 Then
    MsgBox "The dropdown list for selecting Delivery/Financial Unit is full, so the delivery/financial unit information named " & ItemName & " could not be defined.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    Workbooks(FileName).Close SaveChanges:=True
    GoTo Son
End If
...
If SayHedef > 7 Then
    For j = 7 To SayHedef
        If Workbooks(FileName).Worksheets(1).Cells(j, 119).Value = "" Then
            SayHedef = j - 1
            GoTo DonguSon
        End If
    Next j
...
Workbooks(FileName).Worksheets(1).Cells(SayHedef + 1, 119).Value = ItemName
ThisWorkbook.Worksheets(2).Cells(SayHedef + 1, 119).Value = ItemName
...
If SayHedef > 7 Then
    ...
    'Sort A to Z
    Workbooks(FileName).Worksheets(1).Range("DO" & 7 & ":DO" & SayHedef).Sort key1:=Workbooks(FileName).Worksheets(1).Range("DO" & 7), order1:=xlAscending, Header:=xlNo
    ThisWorkbook.Worksheets(2).Range("DO" & 7 & ":DO" & SayHedef).Sort key1:=ThisWorkbook.Worksheets(2).Range("DO" & 7), order1:=xlAscending, Header:=xlNo
...
ComboTeslimatBirimi.Value = ""
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The delivery/financial unit information named " & ItemName & " has been successfully defined for the related dropdown list.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
If ListControl = 0 Then
    MsgBox "The delivery/financial unit information named " & ItemName & " has not been previously defined for the related dropdown list, and therefore the removal operation could not be completed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    GoTo Son
End If
...
Set ItemBul = Workbooks(FileName).Worksheets(1).Range("DO7:DO1000").Find(What:=ItemName, SearchDirection:=xlNext, _
...
Workbooks(FileName).Worksheets(1).Cells(ItemBul.Row, 119).Value = ""
ThisWorkbook.Worksheets(2).Cells(ItemBul.Row, 119).Value = ""
...
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The delivery/financial unit information named " & ItemName & " has been successfully removed from the related dropdown lists.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
For Each ClrLab In support_delivery_units_UI.Controls
...
support_delivery_units_UI.BackColor = RGB(230, 230, 230) 'YENİ
...
Call timeout(0.01)
    If Rep > 60 Then
        support_delivery_units_UI.Width = Rep
        yukseklik = yukseklik - 60
        support_delivery_units_UI.Height = yukseklik
