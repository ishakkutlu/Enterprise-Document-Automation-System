VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} support_financial_themes_UI 
   Caption         =   "Enterprise Document Automation System"
   ClientHeight    =   8895
   ClientLeft      =   30
   ClientTop       =   375
   ClientWidth     =   13920
   OleObjectBlob   =   "support_financial_themes_UI.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "support_financial_themes_UI"
...
Private Sub ComboFinansalBirim_DblClick(ByVal Cancel As MSForms.ReturnBoolean)
Me.ComboFinansalBirim.DropDown
End Sub
...
Private Sub ComboFinansalBirim_MouseMove(ByVal Button As Integer, ByVal Shift As Integer, ByVal x As Single, ByVal y As Single)
Call ColorChangerGenel
Call SetComboBoxHook(ComboFinansalBirim) 'Open scrollable with mouse
End Sub
...
ItemName = ComboFinansalBirim.Value
...
If ItemName <> "" Then
    ...
    a() = ComboFinansalBirim.List
    For i = LBound(a) To UBound(a)
        If a(i, 0) = ItemName Then
            MsgBox ItemName & " named financial unit information has already been defined for the related dropdown list. Therefore, your operation could not be completed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
            GoTo Son
        End If
    Next i
Else
...
SayHedef = Workbooks(FileName).Worksheets(1).Range("DX1000").End(xlUp).Row
If SayHedef < 6 Then
    SayHedef = 6
End If
If SayHedef > 304 Then
    MsgBox "The dropdown list definition area for financial unit selection is full, so the financial unit information named " & ItemName & " could not be defined.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    Workbooks(FileName).Close SaveChanges:=True
    GoTo Son
End If
...
If SayHedef > 6 Then
    For j = 6 To SayHedef
        If Workbooks(FileName).Worksheets(1).Cells(j, 128).Value = "" Then
            SayHedef = j - 1
            GoTo DonguSon
        End If
    Next j
...
Workbooks(FileName).Worksheets(1).Cells(SayHedef + 1, 128).Value = ItemName
ThisWorkbook.Worksheets(2).Cells(SayHedef + 1, 128).Value = ItemName
...
If SayHedef > 6 Then
    ...
    'Sort A to Z
    Workbooks(FileName).Worksheets(1).Range("DX" & 6 & ":DX" & SayHedef).Sort key1:=Workbooks(FileName).Worksheets(1).Range("DX" & 6), order1:=xlAscending, Header:=xlNo
    ThisWorkbook.Worksheets(2).Range("DX" & 6 & ":DX" & SayHedef).Sort key1:=ThisWorkbook.Worksheets(2).Range("DX" & 6), order1:=xlAscending, Header:=xlNo
...
ComboFinansalBirim.Value = ""
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The financial unit information named " & ItemName & " has been successfully defined in the relevant dropdown list.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
If ListControl = 0 Then
    MsgBox "The financial unit information named " & ItemName & " could not be removed because it has not been previously defined in the relevant dropdown list.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    GoTo Son
End If
...
Set ItemBul = Workbooks(FileName).Worksheets(1).Range("DX6:DX1000").Find(What:=ItemName, SearchDirection:=xlNext, _
...
Workbooks(FileName).Worksheets(1).Cells(ItemBul.Row, 128).Value = ""
ThisWorkbook.Worksheets(2).Cells(ItemBul.Row, 128).Value = ""
...
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The financial unit information named " & ItemName & " has been successfully removed from the relevant dropdown lists.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
For Each ClrLab In support_financial_themes_UI.Controls
...
support_financial_themes_UI.BackColor = RGB(230, 230, 230) 'YENİ
...
Call timeout(0.01)
    If Rep > 60 Then
        support_financial_themes_UI.Width = Rep
        yukseklik = yukseklik - 60
        support_financial_themes_UI.Height = yukseklik
