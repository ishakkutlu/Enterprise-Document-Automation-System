VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} support_ID_types_UI 
   Caption         =   "Enterprise Document Automation System"
   ClientHeight    =   8895
   ClientLeft      =   30
   ClientTop       =   375
   ClientWidth     =   13920
   OleObjectBlob   =   "support_ID_types_UI.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "support_ID_types_UI"
...
Private Sub ComboKimlikTipi_DblClick(ByVal Cancel As MSForms.ReturnBoolean)
Me.ComboKimlikTipi.DropDown
End Sub
...
Private Sub ComboKimlikTipi_MouseMove(ByVal Button As Integer, ByVal Shift As Integer, ByVal x As Single, ByVal y As Single)
Call ColorChangerGenel
Call SetComboBoxHook(ComboKimlikTipi) 'Open scrollable with mouse
End Sub
...
If ItemName <> "" Then
    ...
    For i = LBound(a) To UBound(a)
        If a(i, 0) = ItemName Then
            MsgBox "The identity type '" & ItemName & "' is already defined in the dropdown list. The operation could not be completed.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
            GoTo Son
        End If
    Next i
Else
...
End If
If SayHedef > 54 Then
    MsgBox "The identity type '" & ItemName & "' could not be defined because the dropdown list for identity type selection is full.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    Workbooks(FileName).Close SaveChanges:=True
    GoTo Son
End If
...
If SayHedef > 6 Then
    ...
    'Sort A to Z
    Workbooks(FileName).Worksheets(1).Range("DW" & 6 & ":DW" & SayHedef).Sort key1:=Workbooks(FileName).Worksheets(1).Range("DW" & 6), order1:=xlAscending, Header:=xlNo
    ThisWorkbook.Worksheets(2).Range("DW" & 6 & ":DW" & SayHedef).Sort key1:=ThisWorkbook.Worksheets(2).Range("DW" & 6), order1:=xlAscending, Header:=xlNo
...
ComboKimlikTipi.Value = ""
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The identity type named '" & ItemName & "' has been successfully added to the corresponding dropdown list.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
If ListControl = 0 Then
    MsgBox "The identity type named '" & ItemName & "' could not be removed because it was not previously defined in the corresponding dropdown list.", vbOKOnly + vbExclamation, "Enterprise Document Automation System"
    GoTo Son
End If
...
Set ItemBul = Workbooks(FileName).Worksheets(1).Range("DW6:DW1000").Find(What:=ItemName, SearchDirection:=xlNext, _
...
Workbooks(FileName).Worksheets(1).Cells(ItemBul.Row, 127).Value = ""
...
Workbooks(FileName).Save
'ThisWorkbook.Save
MsgBox "The identity type named '" & ItemName & "' has been successfully removed from the corresponding dropdown lists.", vbOKOnly + vbInformation, "Enterprise Document Automation System"
...
For Each ClrLab In support_ID_types_UI.Controls
...
support_ID_types_UI.BackColor = RGB(230, 230, 230) 'YENİ
...
Call timeout(0.01)
    If Rep > 60 Then
        support_ID_types_UI.Width = Rep
        yukseklik = yukseklik - 60
        support_ID_types_UI.Height = yukseklik
